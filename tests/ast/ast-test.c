...
{
    TNode* root = init_tnode(NodeBody, screate("<body>"), NULL, false);
    ASSERT_TRUE(is_root_tnode(root));
    ASSERT_TRUE(is_leaf_tnode(root));
    ASSERT_EQUAL(0, get_tnode_lvl(root));
    TNode* node = init_tnode(NodeHeadingInline, screate("<h1>"), screate("Header content"), false);
    add_tnode(root, node);
    for (size_t i = 0; i < get_array_length(root->children); i++)
    {
        ASSERT_STR("<h1>", sraw(root->children[i]->head));
        ASSERT_STR("Header content", sraw(root->children[i]->content));
    }
...
CTEST(ast, tree_build)
{
    ...
    {
        add_tnode(root, init_tnode(NodeHeadingInline, screate("<h1>"), screate("test"), false));
    }
    for (size_t i = 0; i < get_array_length(root->children); i++)
    {
        ASSERT_STR("<h1>", sraw(root->children[i]->head));
        ASSERT_STR("test", sraw(root->children[i]->content));
    }
    free_tnode(root);
}
